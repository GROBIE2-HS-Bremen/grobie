...
class BH1750_base:
    ...
    def __init__(self, address, i2c):
        ...
        self._i2c = i2c
        self._measurement_mode = BH1750_base.MEASUREMENT_MODE_ONE_TIME
        self._resolution = BH1750_base.RESOLUTION_HIGH
        self._measurement_time = BH1750_base.MEASUREMENT_TIME_DEFAULT
    ...
    def configure(self, measurement_mode: int, resolution: int, measurement_time: int):
        ...
        """
        if measurement_time not in range(BH1750_base.MEASUREMENT_TIME_MIN, BH1750_base.MEASUREMENT_TIME_MAX + 1):
            raise ValueError("measurement_time must be between {0} and {1}"
                             .format(BH1750_base.MEASUREMENT_TIME_MIN, BH1750_base.MEASUREMENT_TIME_MAX))
    ...
    def _write_measurement_mode(self):
        ...
        self._i2c.writeto(self._address, buffer)
        sleep_ms(24 if self._measurement_time == BH1750_base.RESOLUTION_LOW else 180)
    ...
    def measurement(self) -> float:
        ...
        if self._measurement_mode == BH1750_base.MEASUREMENT_MODE_ONE_TIME:
            self._write_measurement_mode()
            
        buffer = bytearray(2)
        self._i2c.readfrom_into(self._address, buffer)
        lux = (buffer[0] << 8 | buffer[1]) / (1.2 * (BH1750_base.MEASUREMENT_TIME_DEFAULT / self._measurement_time))
        
        if self._resolution == BH1750_base.RESOLUTION_HIGH_2:
            return lux / 2
        else:
            return lux
    ...
    def measurements(self):
        ...
        while True:
            ...
            if self._measurement_mode == BH1750_base.MEASUREMENT_MODE_CONTINUOUSLY:
                base_measurement_time = 16 if self._measurement_time == BH1750_base.RESOLUTION_LOW else 120
                sleep_ms(math.ceil(base_measurement_time * self._measurement_time / BH1750_base.MEASUREMENT_TIME_DEFAULT))