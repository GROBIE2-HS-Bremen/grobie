...
def find_prime_polys(generator=2, c_exp=8, fast_primes=False, single=False):
    ...
    if fast_primes:
        ...
        prim_candidates = array.array('i',
                                      [x for x in prim_candidates if x > field_charac])  # filter out too small primes
    else:
        prim_candidates = array.array('i', xrange(field_charac + 2, field_charac_next,
    ...
    for prim in prim_candidates:  # try potential candidates primitive irreducible polys
        ...
        if not conflict:
            ...
            if single: return array.array('i', [
                prim])  # for API consistency, we always return an array, but here with a single value
...
def gf_poly_div(dividend, divisor):
    ...
    for i in xrange(len(dividend) - (divisor_len - 1)):
        ...
        if coef != 0:  # log(0) is undefined, so we need to avoid that case explicitly (and it's also a good optimization). In fact if you remove it, it should still work because gf_mul() will take care of the condition. But it's still a good practice to put the condition here.
            for j in xrange(1,
                            divisor_len):  # in synthetic division, we always skip the first coefficient of the divisior, because it's only used to normalize the dividend coefficient
...
def rs_generator_poly_all(max_nsym, fcr=0, generator=2):
    ...
    g_all = [[
                 1]] * max_nsym  # a list of list is potentially faster than using a dict, since it is pre-allocated and a list has less overhead than a dict
...
def rs_encode_msg(msg_in, nsym, fcr=0, generator=2, gen=None):
    ...
    msg_out = _bytearray(msg_in) + _bytearray(
        len(gen) - 1)  # init msg_out with the values inside msg_in and pad with len(gen)-1 bytes (which is the number of ecc symbols).
    ...
    for i in xrange(msg_in_len):
        ...
        if coef != 0:  # log(0) is undefined, so we need to manually check for this case. There's no need to check the divisor here because we know it can't be 0 since we generated it.
            ...
            for j in xrange(1,
                            gen_len):  # in synthetic division, we always skip the first coefficient of the divisior, because it's only used to normalize the dividend coefficient (which is here useless since the divisor, the generator polynomial, is always monic)
...
def rs_correct_errata(msg_in, synd, err_pos, fcr=0,
    ...
    coef_pos = _bytearray(len(msg) - 1 - p for p in
                          err_pos)  # need to convert the positions to coefficients degrees for the errata locator algo to work (eg: instead of [0, 1, 2] it will become [len(msg)-1, len(msg)-2, len(msg) -3]), also can use a _bytearray() because length of message - and hence length of coef_pos - will always be constrained by the Galois Field.
    ...
    E = _bytearray(
        len(msg))  # will store the values that need to be corrected (substracted) to the message containing errors. This is sometimes called the error magnitude polynomial.
    X_len = len(X)
...
def rs_find_error_locator(synd, nsym, erase_loc=None, erase_count=0):
    ...
    for i in xrange(
        ...
        delta = synd[K]
        for j in xrange(1,
                        len(err_loc)):  # range 1:256 is important: if you use range 0:255, if the last byte of the ecc symbols is corrupted, it won't be correctable! You need to use the range 1,256 to include this last byte.
    ...
    for i, x in enumerate(
            err_loc):  # drop leading 0s, else errs will not be of the correct size. This does not use functional closures (ie, lambdas), which Cython and JIT compilers cannot optimize, such as `err_loc = list(itertools.dropwhile(lambda x: x == 0, err_loc))`
        if x != 0:
            err_loc = err_loc[i:]
            break
    errs = len(
        err_loc) - 1  # -1 because range is 1:256, it's offset by 1, it's not 0:255, hence the length would be overestimated without -1
...
def rs_find_error_evaluator(synd, err_loc, nsym):
    ...
    remainder = remainder[len(remainder) - (
                nsym + 1):]  # then divide by a polynomial of the length we want, which is equivalent to slicing the list (which represents the polynomial)
...
def rs_forney_syndromes(synd, pos, nmess, generator=2):
    ...
    erase_pos_reversed = _bytearray(
        nmess - 1 - p for p in pos)  # prepare the coefficient degree positions (instead of the erasures positions)
...
class RSCodec(object):
    ...
    def encode(self, data, nsym=None):
        ...
        total_chunks = int(math.ceil(float(len(data)) / float(
            chunk_size)))  # need to convert to floats first to get an accurate floating division, or else we assume implicit conversion and it will cause an error on Python 2

        # Preallocate output array
        enc = _bytearray(
            total_chunks * nsize)  # pre-allocate array and we will overwrite data in it, much faster than extending
        # Chunking loop
        for i in xrange(0, total_chunks):
            ...
            enc[i * nsize:(i + 1) * nsize] = rs_encode_msg(data[i * chunk_size:(i + 1) * chunk_size], nsym, fcr=fcr,
    ...
    def decode(self, data, nsym=None, erase_pos=None, only_erasures=False):
        ...
        dec = _bytearray(
            total_chunks * nmes)  # pre-allocate array and we will overwrite data in it, much faster than extending
        ...
        for i in xrange(0, total_chunks):  # Split the long message in a chunk
            ...
            rmes, recc, errata_pos = rs_correct_msg(data[i * chunk_size:(i + 1) * chunk_size], nsym, fcr=fcr,
                                                    generator=generator, erase_pos=e_pos, only_erasures=only_erasures)
            dec[i * nmes:(i + 1) * nmes] = rmes
            dec_full[i * nsize:(i + 1) * nsize] = rmes
            dec_full[i * nsize + nmes:(
                                                  i + 1) * nsize + nmes] = recc  # append corrected ecc just after corrected message. The two lines are equivalent to rmes + recc but here we don't need to concatenate both arrays first (and create a third one for nothing) before storing in the output array
    ...
    def check(self, data, nsym=None):
        ...
        check = [
                    False] * total_chunks  # TODO: could use a bitarray but this creates an external dependency and it's not compatible with cython https://pypi.org/project/bitarray/ and https://www.noveltech.dev/booleans-python-numpy/
        # Chunking loop
        for i in xrange(0, total_chunks):  # Split the long message in a chunk
